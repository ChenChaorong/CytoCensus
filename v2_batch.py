...
import pdb
from common_navigation import navigation_setup,create_channel_objects,btn_fn
from parameter_object import ParameterClass
from File_handler import Intermediate_handler
...
import v2_batch_functions as v2
...
class Eval_load_model_win(QtGui.QWidget):
    ...
    def gotoEvalButton_fn(self):
        ...
        par_obj.tpt_list = range(0,par_obj.max_t+1) #TODO make sure this matches up and don't have a +1 error
...
class Eval_disp_im_win(QtGui.QWidget):
    ...
    def __init__(self,par_obj):
        ...
        self.image_num_txt.setText('The Current Image is: ' + str(par_obj.curr_z +1)+' and the time point is: '+str(par_obj.curr_t +1))
        self.count_txt = QtGui.QLabel()
        ...
        self.count_all_btn = QtGui.QPushButton('Re-Count Maxima')
        self.count_all_btn.clicked.connect(self.count_all_fn)

        self.toggle_display_btn = QtGui.QPushButton('Show Probability/Maxima')
        ...
        self.abs_thr_txt = QtGui.QLineEdit(str(par_obj.abs_thr*100))
        ...
        self.activate_roi_btn = QtGui.QPushButton('Toggle ROI tool')
        self.activate_roi_btn.clicked.connect(self.activate_roi_btn_fn)

        self.compl_btn = QtGui.QPushButton('Save ROI')
        self.compl_btn.clicked.connect(self.cursor.complete_roi)

        self.clear_btn = QtGui.QPushButton('Clear ROI')
        self.clear_btn.clicked.connect(self.cursor.clear_ROI)

        self.interpolate_btn = QtGui.QPushButton('Interpolate (z)')
        self.interpolate_btn.clicked.connect(self.interpolate_roi_fn)

        self.interpolate_time_btn = QtGui.QPushButton('Interpolate (t)')
    ...
    def count_all_fn(self):
        ...
        par_obj.data.hessians = []
        for fileno in range(par_obj.max_file):
            imfile=par_obj.filehandlers[fileno]
            par_obj.data.hessians.append(Intermediate_handler(imfile.full_name+'Hessians.tif'))
            for tpt in range(imfile.max_t+1):
                self.report_progress('Calculating Maxima for File:'+str(fileno+1)+ ' Timepoint: '+str(tpt+1) +' All  Z')
    ...
    def count_maxima_btn_fn(self):
        ...
        par_obj.data.hessians = []
        par_obj.data.hessians.append(Intermediate_handler(par_obj.filehandlers[par_obj.curr_file].full_name+'Hessians.tif'))
    ...
    def evaluate_images(self): 
        par_obj.data.predictions = []
        par_obj.data.hessians = []
        par_obj.maxPred = 0 #resets scaling for display between models
        par_obj.minPred = 100
        for fileno,imfile in par_obj.filehandlers.iteritems():
            
            par_obj.data.predictions.append(Intermediate_handler(imfile.full_name+'Predictions.tif'))
            with par_obj.data.predictions[fileno].writer() as predwriter:
                for tpt in range(imfile.max_t+1):
                    #pool = ThreadPool(8)
                    #pool.map(functools.partial(v2.calculate_feat_and_predict,par_obj, tpt, fileno, 0, predwriter,), range(imfile.max_z+1))
                    for z in range(imfile.max_z+1):
                        v2.calculate_feat_and_predict(par_obj, tpt, fileno, 0, predwriter,z)
                        app.processEvents()
                        time.sleep(0.001)
                    #pool.close()
                    #pool.join()
...
class File_Dialog(QtGui.QMainWindow):
    ...
    def showDialog(self):
        ...
        for path in QtGui.QFileDialog.getOpenFileNames(self, 'Open file', self.par_obj.filepath,'Images(*.tif *.tiff);;'):
