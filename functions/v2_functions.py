...
def save_roi_fn(par_obj):
    ...
    if par_obj.rect_w != 0 and par_obj.rect_h != 0:
        ...
        par_obj.rects = (
            ...
            int(s_ori_x),
            int(s_ori_y),
            int(abs(par_obj.rect_w)),
            int(abs(par_obj.rect_h)),
            par_obj.curr_t,
            par_obj.curr_file,
            par_obj.file_array[par_obj.curr_file],
        )
        return True
    ...
    for it in range(binlength):
        if samples_at_tiers[it] > 0:
            ...
            if stratified_indices[0].__len__() != 0:
                stratified_sampled_indices = np.random.choice(
                    stratified_indices[0], size=samples_at_tiers[it], replace=True, p=None
                )
            else:
...
def update_training_samples_fn_new_only(par_obj, int_obj, rects, arr="feat_arr"):
    ...
    imhist[1][
        binlength
    ] = 5  # adjust top bin to make sure we include everthing if we have overlapping gaussians-try to avoid though-if very common will distort bins
    ...
    if par_obj.p_size == 1:
        ...
        im_region = par_obj.data_store[arr][imno][tpt][zslice][
            rects[2] + 1 : rects[2] + rects[4], rects[1] + 1 : rects[1] + rects[3], :
        ]
        p_region = par_obj.data_store["dense_arr"][imno][tpt][zslice][
            rects[2] + 1 : rects[2] + rects[4], rects[1] + 1 : rects[1] + rects[3]
        ]
            ...
            if par_obj.limit_ratio is True:
                ...
                if STRATIFY is True:
                    ...
                    indices = np.random.choice(
                        int(im_region.shape[0] * im_region.shape[1]), size=int(par_obj.limit_size), replace=True, p=None
                    )
            else:
...
def update_training_samples_fn_auto(par_obj, int_obj, rects):
    ...
    if par_obj.p_size == 1:
        # if rects[5] == tpt and rects[0] == zslice and rects[6] == imno:
        # Finds and extracts the features and output density for the specific regions.
        mImRegion = par_obj.data_store["double_feat_arr"][imno][tpt][zslice][
            rects[2] + 1 : rects[2] + rects[4], rects[1] + 1 : rects[1] + rects[3], :
        ]
        denseRegion = par_obj.data_store["dense_arr"][imno][tpt][zslice][
            rects[2] + 1 : rects[2] + rects[4], rects[1] + 1 : rects[1] + rects[3]
        ]
        # Find the linear form of the selected feature representation
        ...
        if par_obj.limit_sample is True:
            if par_obj.limit_ratio is True:
                ...
                if STRATIFY is True:
                    ...
                    indices = np.random.choice(
                        int(mImRegion.shape[0] * mImRegion.shape[1]), size=int(par_obj.limit_size), replace=True, p=None
                    )
            else:
                # this works because the first n indices refer to the full first two dimensions, and np indexing takes slices
                indices = np.random.choice(
...
def update_com_fn(par_obj, tpt, zslice, fileno):
    ...
    kernel = filters.gaussian_filter(
        patch.astype(np.float32), float(par_obj.sigma_data), order=0, output=None, mode="constant", cval=0.0
    )
    ...
    if threaded == 0:
        ...
        for tpt in tptList:
            for imno in imnoList:
                ...
                pool = ThreadPool(8)
                featlist = pool.map(
                    functools.partial(lf.feature_create_threadable_auto, par_obj, imno, tpt), zsliceList
                )
                pool.close()
                pool.join()
                tee2 = time.time()
                ...
                for zslice in zsliceList:
                    ...
                    par_obj.data_store["double_feat_arr"][imno][tpt][zslice] = np.concatenate(
                        (feat, par_obj.data_store["feat_arr"][imno][tpt][zslice]), axis=2
                    )
        ...
        for tpt in tptList:
            for imno in imnoList:
                ...
                int_obj.report_progress(
                    "Calculating Features for File:" + str(imno + 1) + " Timepoint: " + str(tpt + 1) + " Z: " + "ALL"
                )
                featlist = []
                tee1 = time.time()
                pool = ThreadPool(8)
...
def evaluate_forest_new(
    par_obj, int_obj, withGT, model_num, zsliceList, tptList, curr_file, arr="feat_arr", threaded=False
):
    ...
    for imno in curr_file:
        for tpt in tptList:
            for zslice in zsliceList:
                ...
                if par_obj.p_size == 1:
                    mimg_lin = np.reshape(
                        par_obj.data_store[arr][imno][tpt][zslice],
                        (par_obj.height * par_obj.width, par_obj.data_store[arr][imno][tpt][zslice].shape[2]),
                    )
                if par_obj.p_size == 2:
                ...
                print(
                    "prediction time taken", t1 - t2, " Predicted i:", par_obj.data_store["sum_pred"][imno][tpt][zslice]
                )
                int_obj.report_progress(
                    "Making Prediction for File: " + str(imno + 1) + " T: " + str(tpt + 1) + " Z: " + str(zslice + 1)
                )
    ...
    for imno in curr_file:
        for tpt in tptList:
            for zslice in zsliceList:
                ...
                """
                mimg_lin = np.reshape(
                    par_obj.data_store["double_feat_arr"][imno][tpt][zslice],
                    (par_obj.height * par_obj.width, par_obj.data_store["double_feat_arr"][imno][tpt][zslice].shape[2]),
                )
                t2 = time.time()
...
def goto_img_fn_new(par_obj, int_obj, keep_roi=False):
    ...
    # update image text
    int_obj.image_num_txt.setText(
        "Current File is : "
        + str(par_obj.curr_file + 1)
        + "/"
        + str(par_obj.max_file)
        + ", Current Time: "
        + str(par_obj.curr_t + 1)
        + "/"
        + str(par_obj.max_t + 1)
        + ", Current Z: "
        + str(par_obj.curr_z + 1)
        + "/"
        + str(par_obj.max_z + 1)
    )
...
def eval_pred_show_fn(par_obj, int_obj, zslice, tpt):
    ...
    if par_obj.eval_load_im_win_eval == True:
        ...
        int_obj.image_num_txt.setText(
            "The Current Image is No. " + str(zslice + 1) + " and the time point is: " + str(tpt + 1)
        )
        ...
        for tpt in range(imfile.max_t):
            for zslice in range(imfile.max_z + 1):
                if zslice in par_obj.data_store["dense_arr"][fileno][tpt]:
                    image[tpt, zslice, 0, :, :] = par_obj.data_store["dense_arr"][fileno][tpt][zslice].astype(
                        np.float32
                    )
...
def save_output_ROI(par_obj, int_obj):
    ...
    with open(imfile.path + "/" + par_obj.modelName + "_outputROI.csv", "wb") as csvfile:
        ...
        spamwriter.writerow(
            [str("Filename: ")] + [str("Time point: ")] + [str("Z: ")] + [str("Regions(x): ")] + [str("Regions(y): ")]
        )
...
def save_output_data_fn(par_obj, int_obj):
    ...
    with open(csvPath + "_" + par_obj.modelName + "outputData.csv", "a") as csvfile:
        ...
        spamwriter.writerow(
            [str(par_obj.selectedModel)] + [str("Filename: ")] + [str("Time point: ")] + [str("Predicted count: ")]
        )
        ...
        for fileno, imfile in par_obj.filehandlers.items():
            ...
            for tpt in par_obj.tpt_list:
                spamwriter.writerow(
                    [local_time] + [str(filename)] + [tpt + 1] + [par_obj.data_store["pts"][fileno][tpt].__len__()]
                )
    ...
    for fileno, imfile in par_obj.filehandlers.items():
        with open(csvPath + imfile.name + "_" + par_obj.modelName + "_outputPoints.csv", "a") as csvfile:
            ...
            for tpt in par_obj.tpt_list:
                ...
                for i in range(0, par_obj.data_store["pts"][fileno][tpt].__len__()):
                    spamwriter.writerow(
                        [local_time] + [str(filename)] + [tpt + 1] + [pts[i][0]] + [pts[i][1]] + [pts[i][2]]
                    )
...
def save_user_ROI(par_obj, int_obj):
    ...
    for fileno, imfile in par_obj.filehandlers.items():
        ...
        with open(imfile.path + "/" + filename + "_outputROI.pickle", "wb") as afile:
            data = [
                par_obj.data_store["roi_stkint_x"][fileno],
                par_obj.data_store["roi_stkint_y"][fileno],
                par_obj.data_store["roi_stk_x"][fileno],
                par_obj.data_store["roi_stk_y"][fileno],
            ]
